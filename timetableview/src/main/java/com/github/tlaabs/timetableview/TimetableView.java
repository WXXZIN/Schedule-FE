...
public class TimetableView extends LinearLayout {
    ...
    private void add(final ArrayList<Schedule> schedules, int specIdx) {
        ...
        for (Schedule schedule : schedules) {
            ...
            String classTitle = schedule.getClassTitle();
            String classPlace = schedule.getClassPlace();
            String fullText = classTitle + "\n" + classPlace;

            SpannableString spannable = new SpannableString(fullText);

            int titleStart = 0;
            int titleEnd = classTitle.length();
            spannable.setSpan(new AbsoluteSizeSpan(DEFAULT_STICKER_FONT_SIZE_DP, true), titleStart, titleEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new StyleSpan(Typeface.BOLD), titleStart, titleEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

            int placeStart = classTitle.length() + 1;
            int placeEnd = fullText.length();
            spannable.setSpan(new AbsoluteSizeSpan(DEFAULT_STICKER_FONT_SIZE_DP - 3, true), placeStart, placeEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new StyleSpan(Typeface.NORMAL), placeStart, placeEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

            spannable.setSpan(new ForegroundColorSpan(Color.parseColor("#FFFFFF")), 0, fullText.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
